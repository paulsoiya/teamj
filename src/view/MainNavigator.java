 ...
 *
 * All methods on the navigator are static to facilitate simple access from anywhere in
 * the application.
 */
public class MainNavigator {
	...
	/**
	 * Loads the vista specified by the fxml file into the vistaHolder pane of the main
	 * application layout.
	 *
	 * Previously loaded vista for the same fxml file are not cached. The fxml is loaded
	 * anew and a new vista node hierarchy generated every time this method is invoked.
	 *
	 * A more sophisticated load function could potentially add some enhancements or
	 * optimizations, for example: cache FXMLLoaders cache loaded vista nodes, so they can
	 * be recalled or reused allow a user to specify vista node reuse or new creation
